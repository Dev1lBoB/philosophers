...
/*   Updated: 2021/04/19 22:15:31 by wpersimm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_1(t_all *all, pthread_t **thread)
{
	*thread = malloc(sizeof(pthread_t) * all->size);
	if (!*thread)
		return (1);
	all->mutex = malloc(sizeof(pthread_mutex_t) * all->size);
	if (!all->mutex)
		return (1);
...
void	ft_init_2(t_all *all)
{
	...
	pthread_mutex_init(&all->muta, NULL);
	pthread_mutex_init(&all->mussage, NULL);
	j = -1;
	while (++j < all->size)
		pthread_mutex_init(&all->mutex[j], NULL);
	all->i = -1;
